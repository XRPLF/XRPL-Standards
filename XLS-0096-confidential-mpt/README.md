...
- **Compact sigma proofs**: A compact cryptographic proof that simultaneously validates multiple properties in a single fixed-size bundle — for example, that all encrypted copies of a transfer amount (for the sender, receiver, issuer, and auditor) actually contain the same value, and that the sender's balance commitment matches their on-ledger encrypted balance. Bundling all of these checks into one proof keeps the proof size small and constant regardless of how many recipients are involved.
  ...
  - **Compact ConvertBack sigma proof (128 bytes)**: Used in `ConfidentialMPTConvertBack`. Proves the sender owns the spending balance and that their balance commitment is correctly derived from it.
  - **Compact Send sigma proof (192 bytes)**: Used in `ConfidentialMPTSend`. Proves all encrypted copies of the transfer amount (sender, receiver, issuer, optional auditor) contain the same value, and that the sender's balance commitment matches their on-ledger spending balance.
...
| `ZKProof`                    | Yes       | `string`  | `BLOB`        | N/A           | A 946-byte bundle containing a **compact AND-composed sigma proof** (192 bytes, proving ciphertext consistency across all recipients and balance linkage) and an **aggregated Bulletproof range proof** (754 bytes, proving both the transfer amount and remaining balance are non-negative). |
