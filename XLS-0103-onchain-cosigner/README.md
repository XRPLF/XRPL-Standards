...
Because the ID depends only on `(Owner, target account, sequence/ticket)` and not on the payload contents, an `Owner` **cannot** hold two live proposals that target the same account with the same sequence/ticket, *even if the payloads differ*: the second `TransactionProposalCreate` produces the same `ProposalID` and fails with `tecDUPLICATE` (§5.3.2). This is intentional — both proposals would attempt to consume the same target `Sequence`/`TicketSequence`, so at most one could ever execute. Distinct owners may each hold a proposal for the same triple, since their IDs differ by `Owner`.
...
**Reserve Requirement:** Standard. This ledger entry requires the standard owner reserve increment (currently 0.2 XRP, subject to Fee Voting changes), held against the proposer's account and released when the object is deleted.
...
- **Owner cancellation (non-terminal):** while the proposal is not terminal, only the **`Owner`** (the proposer) may delete it, via `TransactionProposalCancel`.
- **Permissionless cleanup (terminal):** once the proposal is terminal, **any** account may delete it, via either `TransactionProposalCancel` or `TransactionProposalSign`. In the terminal case a `TransactionProposalSign` records no signature; it simply removes the object and releases the owner's reserve.
...
This is the name used in `account_objects` and `ledger_data` RPC calls to filter for this object type. A proposal can also be looked up directly with `ledger_entry` by its `ProposalID`.
...
Appends one signer's multi-signature for the proposed transaction to the proposal. The signature is validated exactly as it would be during standard multi-sign, guaranteeing that the collected set remains submittable. If the proposal is already terminal, this transaction instead cleans it up (see §4.5).
...
| `Account`         | ✔️        | string    | ACCOUNT       | N/A                       | The signer contributing the signature (and paying the fee). Must equal `Signer.Account`. |
| `ProposalID`      | ✔️        | string    | HASH256       | N/A                       | The ID of the `TransactionProposal` being signed.                                    |
| `Signer`          | ✔️        | object    | STOBJECT      | N/A                       | The signer's signature entry (see §6.1.1).                                            |

The signer submits their own signature: `Signer.Account` **must** equal `Account`, and the `TransactionProposalSign` transaction is authorized normally by `Account`. There is no relayer path — the account that posts the signature is the account whose signature is being recorded. The **authority** contributed to the proposal comes entirely from the `Signer` object.

#### 6.1.1. `Signer`

`Signer` is a standard `Signer` inner object — `Account`, `SigningPubKey`, and `TxnSignature` — where `TxnSignature` is `SigningPubKey`'s signature over the relevant signing payload:

- For an **ordinary transaction**, the payload is the proposed transaction's standard multi-sign signing data (the transaction excluding its signature fields), with `Signer.Account` appended as the multi-sign suffix.
- For a **`Batch`**, the payload is the `Flags` field and the hashes of `RawTransactions`, exactly as an XLS-56 `BatchSigner` signs, with `Signer.Account` appended as the multi-sign suffix.

Crucially, the multi-sign suffix is `Signer.Account` — **not** any participant account — so for a Batch a single signature by `Signer.Account` is valid for *every* account whose `SignerList` includes `Signer.Account`. This is why one `TransactionProposalSign` suffices even when the signer authorizes several accounts (see §6.1.2).

`Signer.Account` must equal the transaction's `Account`; a signer always submits their own signature.

#### 6.1.2. Signature routing (multiple accounts)

The ledger applies the submitted `Signer` to **every** account in the proposed transaction for which `Signer.Account` is an authorized signer — the signer names no target account, because a `Batch` is atomic (approving it authorizes every account the signer is entitled to authorize):

- **Ordinary transaction:** the only relevant account is the target account (the transaction's `Account`). `Signer` is inserted into `Transaction.Signers`, and `Signer.Account` must be a member of that account's applicable `SignerList`.
- **`Batch`:** the relevant accounts are the outer account and every participant account (any account with an inner transaction in `RawTransactions`). For each such account whose applicable `SignerList` includes `Signer.Account`, the ledger inserts the `Signer` entry into that account's destination — `Transaction.Signers` for the outer account, or the account's entry in `Transaction.BatchSigners` (its nested `Signers` array) for a participant. The single signature is reused across all of them.

A participant account may instead authorize with its **own** master/regular key: submit a `Signer` whose `Account` equals that participant and whose `SigningPubKey` is that account's own key; the ledger records it as a single-signature `BatchSigner` (`SigningPubKey`/`TxnSignature`, no nested `Signers`) for that one account.

The transaction fails only if `Signer.Account` is not an authorized signer for **any** relevant account (see §6.3).
...
1. `ProposalID` or `Signer` is missing or malformed (`temMALFORMED`).
2. `Signer.Account` does not equal the transaction's `Account` (`temMALFORMED`).
3. `Signer.TxnSignature` is not a valid signature by `Signer.SigningPubKey` over the relevant signing payload (§6.1.1) (`temBAD_SIGNATURE`).

#### 6.3.2. Protocol-Level Failures

1. No `TransactionProposal` object exists with the given `ProposalID` (`tecNO_ENTRY`).
2. `Signer.Account` is not an authorized signer (own key or `SignerList` member) for any relevant account of the proposed transaction (`tecNO_PERMISSION`).
3. `Signer.Account` is already recorded for every account it would authorize (fully redundant) (`tecDUPLICATE`).
4. Adding this signature would exceed the maximum of 32 entries in a destination `Signers` array (`tecOVERSIZE`).

_(When the proposal is terminal, this transaction does not fail on the above signer-related conditions; instead it performs cleanup — see §6.4.)_

### 6.4. State Changes

**On Success (`tesSUCCESS`), proposal not terminal:**

- Validates `Signer` and inserts it into the destination for every account it authorizes (§6.1.2), keeping any `Signers` array sorted by `Account`.
...
**On Success (`tesSUCCESS`), proposal terminal:**

- Records no signature. Deletes the `TransactionProposal` object and decrements the proposer's `OwnerCount` (releasing the reserve). This is permissionless.
...
Signing a proposed multi-account `Batch`. `rBOBSIGNER`'s single signature (over the Batch `Flags` + `RawTransactions` hashes, suffixed with `rBOBSIGNER`) is inserted into the `BatchSigner.Signers` of **every** participant account whose `SignerList` includes `rBOBSIGNER` — one transaction, no per-account targeting:
...
- **Batch (XLS-56):** The proposed transaction may be a `Batch`, enabling multi-account, atomic, multi-signed settlement (e.g. end-of-day repo netting, flash-style capital operations). Signatures land in the outer `Signers` or in per-account `BatchSigners` automatically, and because a signer's single signature authorizes every account it is entitled to (§6.1.2), a signer on several participants' lists signs just once. Once every participant's requirement is met the completed batch executes atomically. This is the primary motivating case for On-Chain Cosigner, since multi-account Batches otherwise require the most off-chain signature coordination.
...
Each proposal consumes an owner reserve held against the proposer, bounding how many proposals an account can create and providing an economic disincentive to spam. Because every appended signature must be valid, an attacker cannot inflate a proposal with junk. Built-in expiry ensures abandoned proposals can always be cleaned up (by anyone, once terminal) so they do not accumulate indefinitely in ledger state.
...
A signer submits one `TransactionProposalSign` with their signature over the Batch payload (`Flags` + `RawTransactions` hashes). The ledger places that signature into the outer account's `Transaction.Signers` and/or into the `BatchSigners` entry of every participant account whose `SignerList` includes the signer — a single signature covers all of them, because the multi-sign suffix is the signer's own account, not the participant's. A participant may instead authorize with its own key as a single-signature `BatchSigner`. The proposal is complete once the outer account's quorum and every participant's `BatchSigner` are satisfied; the `Transaction` field is then a fully-signed Batch ready to submit. See §6.1.2.
