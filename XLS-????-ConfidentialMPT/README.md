...
A `ConfidentialMPTNote` is a standalone ledger object representing an encrypted token value. It includes:

- A **commitment** to the value (using ElGamal or Pedersen form)
- A **stealth address** (used as the recipient’s key)
- A **key image** to prevent double spending
- A **spent flag** indicating whether the note has been used

#### Fields

- `LedgerEntryType`: `"ConfidentialMPTNote"`
- `Issuer`: Issuer of the MPT
- `Currency`: MPT currency code
- `Commitment`: EC-ElGamal or Pedersen commitment to the value
- `StealthAddress`: One-time public key for the recipient
- `EphemeralKey`: Sender’s ephemeral key used for stealth derivation
- `KeyImage`: Unique identifier for preventing double spending
- `Spent`: Boolean flag (default `false`)

---

### Transaction Type: `ConfidentialMPTTransfer`

Transfers one or more `ConfidentialMPTNote` objects using **ring signatures** and creates new notes for recipients.

| Field              | Type   | Description                                                     |
|-------------------|--------|-----------------------------------------------------------------|
| `TransactionType` | String | `"ConfidentialMPTTransfer"`                                     |
| `Ring`            | List   | List of public keys forming the anonymity set                   |
| `KeyImage`        | Blob   | Unique identifier of the spent note (prevents reuse)            |
| `RingSignature`   | Blob   | Linkable ring signature proving knowledge of one private key    |
| `Inputs`          | List   | Identifiers (hashes) of `ConfidentialMPTNote` objects to spend   |
| `NewNotes`        | List   | New `ConfidentialMPTNote` objects to create                     |
| `BalanceProof`    | Blob   | ZKP: input commitments equal sum of output commitments          |
| `RangeProofs`     | List   | ZKPs proving each new note amount is non-negative and valid     |
| `Issuer`          | Account | MPT issuer (shared by all notes in the transaction)            |
| `Currency`        | String | MPT currency code                                               |
...
- **Signature Verification**:
  - Check that `RingSignature` is valid for one of the public keys in the `Ring`.
  - Ensure `KeyImage` is not already present in the ledger (prevents double spending).

- **Input Validation**:
  - Ensure each referenced `ConfidentialMPTNote` exists and is not spent.
  - Confirm all notes share the same `Issuer` and `Currency`.

- **ZKP Validation**:
  - Verify `BalanceProof`: sum(inputs) = sum(outputs)
  - Verify `RangeProofs`: all new notes encode non-negative values

- **Ledger Updates**:
  - Mark each input note as `Spent = true`
  - Create new `ConfidentialMPTNote` entries for each item in `NewNotes`
  - Record `KeyImage` to prevent reuse

---

### Ledger Updates

- Each input `ConfidentialMPTNote` is marked as **spent**
- Each output note is added to the ledger as a new `ConfidentialMPTNote`
- A `KeyImage` entry is recorded to prevent double spending
- No account balances or `ConfidentialOutstandingAmount` are modified (value is preserved across notes)

---

### Privacy Properties

- **Sender Anonymity**: Provided by ring signatures. The sender is indistinguishable among the set of public keys in the ring.
