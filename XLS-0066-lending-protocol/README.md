...
The parent protocol is `LendingProtocol` (`featureLendingProtocol`). In failure conditions and state changes, a `LendingProtocol` bullet is the behaviour when `fixCleanup3_4_0` is not enabled; a `fixCleanup3_4_0` bullet is the behaviour when it is.
