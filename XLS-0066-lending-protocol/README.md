...
- `LendingProtocolV1_1` (not yet live, [XLS-66.1](./66.1/README.md)): Vaults created after activation use cash-basis accounting and record `LEVersion = 1`; earlier Vaults remain on accrual-basis accounting. The amendment does not remove `CoverRateLiquidation`; First-Loss Capital keeps the two-rate formula under both accounting models.
- `fixCleanup3_4_0` (not yet live, [XLS-66.2](./66.2/README.md)): Restricts impairment to overdue Loans and stops impairment and unimpairment from rewriting `NextPaymentDueDate`.
...
For a legacy Vault (where `LEVersion` is absent or 0):

$$
DefaultAmount = PrincipalOutstanding + InterestOutstanding_{net} \quad \text{(34a)}
$$

For a Vault with `LEVersion = 1`:

$$
DefaultAmount = PrincipalOutstanding \quad \text{(34b)}
$$
