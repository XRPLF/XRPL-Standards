...
* **Test Plan** (optional): A test plan for an implementation are mandatory for XLSes that are affecting rippled. Tests should either be inlined in the XLS as data (such as input/expected output pairs), or included in ../xls-###/\<filename\>. This section may be omitted for Community and Meta proposals.  
...
Each XLS must begin with an [RFC 822](https://www.ietf.org/rfc/rfc822.txt) style header preamble, contained in a `<pre>` HTML block. This header is also termed [“front matter” by Jekyll](https://jekyllrb.com/docs/front-matter/). The headers must appear in the following order.
...
This section outlines transaction fields, provides details about them, defines special logic, failure conditions, state changes, and invariants. This table should list fields specific to this transaction. Common transaction fields (e.g., `Account`, `Fee`, `Sequence`, `Flags` (common transaction flags), `SigningPubKey`, `TxnSignature`) are assumed unless their usage has special implications for this transaction type.
...
##### 4.4.3.2.4. Invariants
* Invariants are logical statements that MUST be true before and after the execution of a transaction (if successful). They help ensure the transaction does not lead to an invalid or inconsistent ledger state. Use `<object>` for the state before and `<object>'` (see the added grave) for the state after. 
