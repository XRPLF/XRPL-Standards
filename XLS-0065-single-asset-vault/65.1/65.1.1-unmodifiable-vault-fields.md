...
A field added to the `Vault` later is mutable unless it is included in this set. `VaultKind`, `SubscriptionDate` and `RedemptionDate` are included, so the closed-ended vault's phase schedule cannot be changed after creation.
