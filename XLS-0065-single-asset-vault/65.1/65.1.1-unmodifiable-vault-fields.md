<pre>
  ...
  description: Makes the Vault's identity and configuration fields immutable once set
...
Under the `LendingProtocolV1_1` amendment, `Sequence`, `OwnerNode`, `Owner`, `WithdrawalPolicy`, `Scale` and `LEVersion` are immutable on a `Vault` once set, in addition to `Asset`, `Account` and `ShareMPTID`. The closed-ended lifecycle fields are introduced and made immutable by [XLS-65.1.4](./65.1.4-closed-ended-vault.md).
...
A field added to the `Vault` later is mutable unless the patch introducing it adds that field to the invariant. [XLS-65.1.4](./65.1.4-closed-ended-vault.md) does so for `VaultKind`, `SubscriptionDate`, and `RedemptionDate`, keeping the closed-ended schedule rule with the patch that defines those fields.
