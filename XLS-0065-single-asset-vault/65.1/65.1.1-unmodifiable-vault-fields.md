...
A field added to the `Vault` later is mutable unless the patch introducing it adds that field to the invariant. `VaultKind`, `SubscriptionDate` and `RedemptionDate` are included, so the closed-ended vault's phase schedule cannot be changed after creation. [XLS-65.3](../65.3/README.md) does the same for `EarlyExitFeeRate` under `LendingProtocolV1_2`, so the price of an early exit cannot be changed after depositors have committed capital.
