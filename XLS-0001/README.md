...
<img width="900" height="400" alt="image" src="https://github.com/user-attachments/assets/6078be93-b282-41f1-9b8b-d05a2048770c" />
...
- **Backwards Compatibility** (required for Amendments, optional for System/Ecosystem/Meta): All XLSes that introduce backwards incompatibilities for XRPL applications (such as data indexers or on-chain WASM code) must include a section describing these incompatibilities and their consequences. Some examples would be if a field is renamed or a previously-required field is now optional. This section is required for Amendment XLSes, recommended for System XLSes, and optional for Ecosystem and Meta XLSes (though for some it may just note that there is no change).
...
- **Proof-of-Concept Implementation** (optional): An optional section that contains a reference/example implementation that people can use to assist in understanding or implementing this specification. The implementation may be either inlined in the XLS file or included in the `../xls-###/<filename>` directory, where `###` refers to the XLS number.
